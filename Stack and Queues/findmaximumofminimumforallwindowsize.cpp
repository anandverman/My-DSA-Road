...
// The only line of output of each test case should contain ‘N’ space-separated integer such that he ith integer 
// indicates the maximum of minimums of the windows of size ‘i’.
...
//Partial Solution: Gives TLE for big inputs
    
#include <deque>
int minSlidingWindow(vector<int>& arr, int k) 
    {
        deque<int> dq;
        int maxi=INT_MIN;
...
vector<int> maxMinWindow(vector<int> a, int n) {
    vector<int> ans;
    for(int i=1;i<=n;i++)
        ans.push_back(minSlidingWindow(a,i));
    return ans;
}
