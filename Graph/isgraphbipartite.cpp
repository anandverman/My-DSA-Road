...
public:
    bool isBipartite(vector<vector<int>>& graph) {
        ...
        queue<int> q;
        for(int i=0; i<n; i++)
            if(color[i]==0)         //code in this block can also be put in a function
            {
                q.push(i);
                color[i]=1;
                while(!q.empty())
...
class Solution {
public:
    bool dfscoloring(vector<vector<int>>& graph, vector<int> &color, int node)
    {
        for(int i: graph[node])
        {
            if(color[i]==0)
            {
                color[i]=color[i]-color[node];
                if(!dfscoloring(graph, color, i))
                    return false;
            }
            else if(color[i]==color[node])
                return false;
