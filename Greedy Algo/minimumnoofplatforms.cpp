...
int calculateMinPatforms(int at[], int dt[], int n) {
    int ans=0;
    sort(at,at+n);//sort arrival and departure times
    sort(dt,dt+n);
    int i=0,j=0,count=0;
    while(i!=n)
    {
        if(at[i]<=dt[j])
        {
            count++;//count no of trains arrived while one hasn't departed.
            i++;
        }
        else
        {
            j++;
            count-=1;//reduce count of train at the platform as one departed
        }
        ans=max(count,ans);
    }
    return ans;
