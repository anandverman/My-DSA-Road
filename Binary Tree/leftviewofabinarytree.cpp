...
// Where ‘T’ is the number of test cases, and ‘N’ is the total number of nodes in the binary tree, and “data” is the 
...
void preorder(vector<int> &res, TreeNode<int> *root, int index)
{
    if(root == NULL)
        return;
    if(res.size()==index)
        res.push_back(root->data);
    preorder(res, root->left, index+1);
    preorder(res, root->right, index+1);
}
