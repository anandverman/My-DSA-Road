...
// Approach count each swap in mergesort for optimized solution
...
{
    long long invs=0,mid;
    if(right>left)
    {
        mid=(left+right)/2;
        invs+=mergesort(arr,tmp,left,mid);
        invs+=mergesort(arr,tmp,mid+1,right);
        invs+=merge(arr,tmp,left,mid+1,right);
    }
    return invs;
}
