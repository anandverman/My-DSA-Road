// Problem Statement
// You have been given a binary search tree of integers with ‘N’ nodes. You are also given 'KEY' which represents data of a 
// node of this tree. Your task is to find the predecessor and successor of the given node in the BST.
// Note:

// 1. The predecessor of a node in BST is that node that will be visited just before the given node in the inorder traversal 
// of the tree. If the given node is visited first in the inorder traversal, then its predecessor is NULL.
...
// The second line of each test case contains 'KEY' representing the data of the node for which predecessor and successor 
// are to be found.
...
// For each test case, print two single space-separated integers representing data values of the predecessor and the 
// successor node, respectively. If any of the two doesn’t exist, print -1 in place of it.
...
// Since the node with data 8 is on the immediate left of the node with data 10 in the inorder traversal, the node with 
// data 8 is the predecessor.

// Since the node with data 12 is on the immediate right of the node with data 10 in the inorder traversal, the node with 
// data 12 is the successor.
...
{
    pair<int,int> ans;
    BinaryTreeNode<int> *pred=NULL, *succ=NULL;
    pred=inorderPredecessor(root, key);
    succ=inorderSuccessor(root, key);
    ans.first=pred!=NULL?pred->data:-1;
    ans.second=succ!=NULL?succ->data:-1;
    return ans;
}
