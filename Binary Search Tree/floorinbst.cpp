Problem Statement
You are given a BST (Binary search tree) with’ N’ number of nodes and a value ‘X’. Your task is to find the greatest 
value node of the BST which is smaller than or equal to ‘X’.
Note :‘X’ is not smaller than the smallest node of BST .
For example:

In the above example, For the given BST  and X = 7, the greatest value node of the BST  which is smaller than or equal 
to  7 is 6.
...
The first line of each test case contains nodes in level order form (separated by space). If any node does not have a 
...
For each test case, print a single integer ‘M’, denoting the greatest value node of the BST  which is smaller than or 
equal to  ‘X’.
...
//Solution: Similar to Predecessor in BST

TreeNode<int>* predecessor(TreeNode<int>* root, int key)
{
    TreeNode<int>* pred=NULL;
    while(root!=NULL)
    {
        if(root->val>key)   //original predecessor code has >= instead of >
            root=root->left;
        else
        {
            pred=root;
...
{
    TreeNode<int>* floor=predecessor(root, X);
    return floor->val;
}
