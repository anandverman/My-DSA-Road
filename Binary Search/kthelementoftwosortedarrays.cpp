// Problem Statement
// Ninja wants to serve food to needy people. So, he bought Ladoos from a sweet shop and placed them on plates. 
// There can be any number of Ladoos present in a plate.
...
// Now people come one by one in a line to take plates of Ladoos from Ninja. Ninja picks the two plates in front, 
// one from each row and gives that plate to people in which the number of ladoos is the smallest (if both plates 
// contain equal numbers of ladoos then he serves any plate from the two plates) and places the other plate back 
// to its position.

// For Example :
// If ‘ROW1’ is [2, 5, 8, 17] and ‘ROW2’ is [1, 4, 8, 13, 20], then Ninja picks the first plates from each rows, plate 
// containing 2 ladoos from ‘ROW1’ and a plate containing 1 ladoo from ‘ROW2’. 
...
// The first line of each test case contains three single space-separated integers ‘N’,  ‘M’ and ‘K’ where ‘N’ and ‘M’ 
// denote the number of plates containing ladoos in ‘ROW1’ and ‘ROW2’  respectively and ‘K’ denotes the ‘K’th’ person in 
// line waiting to be served.
// The second line of each test case contains ‘N’ single space-separated integers, denoting the number of ladoos in each 
// plate of the first row i.e. ‘ROW1’.
// The third line of each test case contains ‘M’ single space-separated integers, denoting the number of ladoos in each 
// plate of the second row i.e. ‘ROW2’.
...
// 2’nd person will get 3 ladoos i.e now from both rows we will get a plate of 3 ladoos so Ninja can give any one plate 
// containing ladoos from each row. Let us assume Ninja give a plate from ‘ROW2’. Now  ‘ROW1’ :  [3, 7, 10] and  
// ‘ROW2’ :  [5, 5, 7].
